...
/*   Updated: 2024/01/14 12:03:03 by pnopjira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_test_map(t_main *main_struc)
{
	...
	i = 0;
	j = 0;
	main_struc->map->map = malloc(sizeof(int *) * 5);
	while(i < 5)
	{
		j = 0;
		(main_struc->map->map)[i] = malloc(sizeof(int) * 5);
		while(j < 5)
		{
			(main_struc->map->map)[i][j] = test_map[i][j];
			j++;
		}
		i++;
	}
...
void	print_test_map(t_main *main_struc)
{
	...
	while(i < 5)
	{
		...
		while(j < 5)
		{
			printf("%d", (main_struc->map->map)[i][j]);
			j++;
		}
		printf("\n");
		i++;
...
int	main(int argc, char **argv)
{
	...
	mlx_loop(main_struc.viewport.mlx);
	return (0);
}
